...
describe('App, on the desktop', () => {
  ...
  it('hands a dropped file to the bridge, and never derives a path itself (S5)', async () => {
    ...
    const file = new File(['{}'], 'Star Voyager.tms9918')
  ...
  describe('when the open document changes on disk', () => {
    ...
    async function opened(name = 'Star Voyager') {
      const project = createProject({ name, type: 'text' })
    ...
    it('takes the file in place when nothing here is unsaved', async () => {
      ...
      const theirs = createProject({ name: 'From the branch', type: 'text' })
    ...
    it('asks first when there is an unsaved edit, and names both sides', async () => {
      ...
      store.current!.name = 'Star Voyager edited'
      store.markDirty()

      main.changeOnDisk(serializeProject(createProject({ name: 'From the branch', type: 'text' })))
      await flushPromises()
