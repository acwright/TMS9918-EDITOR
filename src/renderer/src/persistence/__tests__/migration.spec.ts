...
function seed(name: string, modifiedAt: string): string {
  const project = { ...createProject({ name, type: 'text' }), modifiedAt }
...
describe('copying', () => {
  it('hands main the git-first document, and keeps the originals (D2, D4)', async () => {
    ...
    expect(outcome.written).toEqual([{ id, file: 'Star Voyager.tms9918' }])
    const text = main.files.get('Star Voyager.tms9918')!
