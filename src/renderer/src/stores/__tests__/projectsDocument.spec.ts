...
describe('projects store, on the desktop', () => {
  ...
  it('names a copy after the open document, not the project inside it', async () => {
    ...
    const project = createProject({ name: 'graphics1', type: 'graphics1' })
    main.seed('Star Voyager', serializeProject(project))
    await store.open(project.id)

    const payload = await store.exportProject(project.id)
    expect(payload?.filename).toBe('Star Voyager.tms9918')
  ...
  it('creates a project as a file and opens it', async () => {
    const store = useProjectsStore()
    const project = await store.create({ name: 'Star Voyager', type: 'graphics1' })

    expect(project).not.toBeNull()
    expect(store.lastError).toBeNull()
    expect(main.document?.path).toBe('/documents/Star Voyager.tms9918')
  ...
  it('shows the document’s name in place of the project list’s', async () => {
    const store = useProjectsStore()
    const project = (await store.create({ name: 'Star Voyager', type: 'text' }))!

    expect(store.documentName).toBe('Star Voyager')
    await store.open(project.id)
    expect(store.documentName).toBe('Star Voyager')
  })
  ...
  it('re-reads the open document rather than keeping a list', async () => {
    ...
    const project = createProject({ name: 'Star Voyager', type: 'graphics1' })
    main.seed('Star Voyager', serializeProject(project))

    const store = useProjectsStore()
    expect((await store.open(project.id))?.id).toBe(project.id)
    expect(store.current?.name).toBe('Star Voyager')
    expect(store.summaries).toEqual([])
  })

  it('answers null when the route names a document that is not open', async () => {
    const store = useProjectsStore()
    await store.create({ name: 'Alpha', type: 'text' })
  ...
  it('refuses to create a second document of the same name', async () => {
    const store = useProjectsStore()
    await store.create({ name: 'Alpha', type: 'text' })
    main.fail('"Alpha.tms9918" already exists in that folder.')

    expect(await store.create({ name: 'Alpha', type: 'text' })).toBeNull()
  ...
  it('takes a document that arrived, and names it', async () => {
    ...
    const project = createProject({ name: 'Star Voyager', type: 'sprite' })
    main.arrive('Star Voyager', serializeProject(project))
  ...
  it('flushes the open document into its own file before taking the next (D17)', async () => {
    vi.useFakeTimers()
    const store = useProjectsStore()
    const first = (await store.create({ name: 'Alpha', type: 'text' }))!
    ...
    const second = createProject({ name: 'Beta', type: 'text' })
  ...
  it('asks for a document rather than being answered with one', async () => {
    ...
    const project = createProject({ name: 'Star Voyager', type: 'sprite' })
    main.stage('Star Voyager', serializeProject(project))
  ...
  it('lists recent documents, and opens one by its id (D16)', async () => {
    const project = createProject({ name: 'Star Voyager', type: 'text' })
    main.seed('Star Voyager', serializeProject(project))

    const store = useProjectsStore()
    const recent = await store.recentDocuments()
    expect(recent.map((entry) => entry.name)).toEqual(['Star Voyager'])
...
describe('the document changing on disk', () => {
  ...
  async function opened(name = 'Alpha') {
    const store = useProjectsStore()
    const project = (await store.create({ name, type: 'text' }))!
    await store.open(project.id)
  ...
  function theBranchVersion(name = 'From the branch') {
    return createProject({ name, type: 'text' })
  }
