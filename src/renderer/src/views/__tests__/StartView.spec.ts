...
describe('StartView', () => {
  ...
  it('routes a sample through the same dialog, named and located', async () => {
    ...
    dialog.vm.$emit('create', { name: 'My Copy', type: 'text' })
    await flushPromises()
  ...
  it('lists recent documents, and asks for one by its opaque id', async () => {
    ...
    expect(list.text()).toContain('Star Voyager')
  ...
  it('offers to copy what is in browser storage, and never moves it', async () => {
    ...
    expect(sheet.props('folder')).toBe('~/Documents/TMS9918 Editor')

    sheet.vm.$emit('copy')
    await flushPromises()

    expect(migration.files.get('Star Voyager.tms9918')).toContain('Star Voyager')
  ...
  it('does not offer one twice', async () => {
    createRepository().save(createProject({ name: 'Star Voyager', type: 'text' }))
    migration.markDone()
