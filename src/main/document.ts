 ...
 *
 * The compound v1 name is tried first, or `Star Voyager.tms9918.json` would
 * come back as `Star Voyager.tms9918` (D3).
 */
 ...
 * the separators and Windows' reserved set — so "Star Voyager" stays
 * `Star Voyager.tms9918` rather than becoming a slug (D3).
 */
...
async function open(parent: BrowserWindow | null): Promise<DocumentResult<OpenDocument>> {
  const options = {
    ...
    filters: [
      ...
      // so `json` is what admits a v1 `.tms9918.json` export (D3).
