...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'tms9918-migration-'))
...
describe('the migration folder', () => {
  ...
  it('is created if it is not there', () => {
    ...
    expect(existsSync(join(documents, MIGRATION_FOLDER_NAME, 'Star Voyager.tms9918'))).toBe(true)
  })
})
...
describe('migrating', () => {
  it('writes one file per project, named as the project is', () => {
    ...
    expect(result.written.map((entry) => entry.file)).toEqual([
      'Star Voyager.tms9918',
      'Tile Set.tms9918',
    ])
    expect(result.failed).toEqual([])
    expect(readFileSync(join(migrationFolder(), 'Star Voyager.tms9918'), 'utf-8')).toContain(
      'Star Voyager',
    )
  })
  ...
  it('suffixes a name twice over rather than overwriting a project', () => {
    ...
    expect(result.written.map((entry) => entry.file)).toEqual([
      'Star Voyager.tms9918',
      'Star Voyager 2.tms9918',
      'Star Voyager 3.tms9918',
    ])
  })
  ...
  it('steps around a file that is already in the folder', () => {
    ...
    expect(result.written[0]!.file).toBe('Star Voyager 2.tms9918')
  ...
  it('seeds recents so the newest project is at the top (D16, D19)', () => {
    ...
    expect(recentDocumentPaths()).toEqual([
      join(migrationFolder(), 'Newest.tms9918'),
      join(migrationFolder(), 'Middle.tms9918'),
      join(migrationFolder(), 'Oldest.tms9918'),
    ])
  })
})
...
describe('the marker', () => {
  ...
  it('names the project that could not be written and keeps the rest', () => {
    ...
    mkdirSync(join(migrationFolder(), 'Tile Set.tms9918.tmp'), { recursive: true })

    const result = migrateDocuments([project('Star Voyager'), project('Tile Set')])

    expect(result.written.map((entry) => entry.file)).toEqual(['Star Voyager.tms9918'])
...
describe('what the renderer is told', () => {
  it('is a folder as it reads on screen, and no path per file (D8)', () => {
    ...
    expect(result.written[0]).toEqual({ id: 'star voyager', file: 'Star Voyager.tms9918' })
  })
})
