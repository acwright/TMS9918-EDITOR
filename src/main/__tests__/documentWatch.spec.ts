 ...
 *
 * The rest is the quiet: a watch that fires for every neighbour in a source
...
beforeEach(() => {
  directory = mkdtempSync(join(tmpdir(), 'tms9918-watch-'))
  document = join(directory, 'Star Voyager.tms9918')
...
describe('watchDocument', () => {
  ...
  it('says nothing about the neighbours', async () => {
    await arm(document)
    writeFileSync(join(directory, 'main.asm'), 'nop', 'utf-8')
    writeFileSync(join(directory, 'Star Voyager.tms9918.tmp'), 'half a write', 'utf-8')
    await settled()
  ...
  it('follows the document to another directory', async () => {
    const elsewhere = mkdtempSync(join(tmpdir(), 'tms9918-watch-'))
    const moved = join(elsewhere, 'Beta.tms9918')
  ...
  it('is quiet about a directory it cannot watch', () => {
    expect(() => watchDocument(join(directory, 'gone', 'Beta.tms9918'), count)).not.toThrow()
    expect(watchedDocument()).toBeNull()
  })
