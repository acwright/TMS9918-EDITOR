...
beforeEach(() => {
  directory = mkdtempSync(join(tmpdir(), 'tms9918-open-'))
...
describe('requestOpen', () => {
  it('holds a document that arrives before the window does (S2)', () => {
    ...
    expect(takePendingDocument()).toContain('Star Voyager.tms9918')
  ...
  it('drops a path that is not there rather than queueing a failure', () => {
    requestOpen(join(directory, 'gone.tms9918'))
    expect(hasPendingDocument()).toBe(false)
  })
})
...
describe('documentFromArgv', () => {
  ...
  it('finds the document a double-click passed on Windows and Linux', () => {
    pretend('linux')
    expect(documentFromArgv(['/opt/app/tms9918-editor', document])).toBe(document)
  })

  it('never reads argv on macOS, where the document comes through open-file (S2)', () => {
    pretend('darwin')
    expect(documentFromArgv(['/Applications/TMS9918 Editor.app', document])).toBeNull()
  })
  ...
  it('ignores a document-shaped argument that is not a file', () => {
    pretend('linux')
    expect(documentFromArgv(['electron', join(directory, 'gone.tms9918')])).toBeNull()
