...
function document(name: string): string {
  const path = join(directory, `${name}.tms9918`)
  writeFileSync(path, '{}\n', 'utf-8')
  return path
}

beforeEach(() => {
  directory = mkdtempSync(join(tmpdir(), 'tms9918-recent-'))
...
describe('recent documents', () => {
  it('puts the newest first', () => {
    ...
    expect(recentDocumentPaths().map((path) => path.endsWith('Beta.tms9918'))).toEqual([
      true,
      false,
    ])
  })
  ...
  it('moves a document already in the list rather than listing it twice', () => {
    ...
    expect(recentDocumentPaths()).toEqual([alpha, join(directory, 'Beta.tms9918')])
  })
  ...
  it('keeps 16, which is what D16 asks for', () => {
    ...
    expect(recentDocumentPaths()[0]).toBe(join(directory, 'Project 19.tms9918'))
  })
  ...
  it('drops a document that is no longer there', () => {
    ...
    expect(recentDocumentPaths()).toEqual([join(directory, 'Beta.tms9918')])
    // And the pruning is written back, not recomputed every time.
    unlinkSync(join(directory, 'Beta.tms9918'))
