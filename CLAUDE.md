...
- **The renderer is served over `app://tms9918/` in production, not `file://`.** Under
...
- **The file association claims `.tms9918`, never `.tms9918.json`.** Windows associates on
...
## The VIC-20 Editor is the same app

`../VIC-EDITOR` is structurally identical — same stack, same layout, same tooling, same
...
chunked by the git-first serializer (a mode's column count here, `settings.columns` there)
and the document key order. `PLAN.md` §9 is the full table. A fix to the shell here is
almost always a fix there too.
